...
export default function VoiceChat() {
    ...
    return (
        <div className="relative group shrink-0 mb-1 mr-1">
            <button
                onClick={isActive ? stopVoiceSession : startVoiceSession}
                title={isActive ? 'Stop Voice' : 'Start Voice'}
                className={`p-4 rounded-2xl shadow-xl flex items-center justify-center transition-all hover:scale-105 active:scale-95 ${isActive ? 'bg-red-500 animate-pulse text-white' : 'bg-[#141413] dark:bg-white text-white dark:text-black'
                    }`}
            >
                {isConnecting ? (
            ...
            <div className="absolute -top-12 left-1/2 -translate-x-1/2 bg-black/80 backdrop-blur-md px-4 py-2 rounded-xl text-[10px] font-black text-white uppercase tracking-widest whitespace-nowrap opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none shadow-2xl z-50">
