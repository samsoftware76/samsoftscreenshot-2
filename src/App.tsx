                ...
                try {
                    ...
                    if (data?.messages) {
                        const formatted = data.messages.map((m: any) => ({
                            ...
                            files: m.metadata?.files // Assuming files are in metadata
                        }));
            ...
            <aside className={`fixed inset-y-0 left-0 bg-white dark:bg-[#111] w-80 shadow-2xl z-50 transform transition-transform duration-300 ease-[cubic-bezier(0.16,1,0.3,1)] flex flex-col ${sidebarOpen ? 'translate-x-0' : '-translate-x-full'}`}>
                <div className="p-6 flex flex-col h-full w-full">
                    ...
                    <div className="flex-1 overflow-y-auto space-y-2 custom-scrollbar">
                        <p className="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-4">Chat History</p>
                        {sessions.map(s => (
                            <button
                                key={s.id}
                                onClick={() => {
                            ...
                            >
                                <span className="text-[11px] font-bold truncate">{s.title || 'Untitled Solve'}</span>
                                <span className="text-[8px] font-black uppercase tracking-tighter opacity-50">{s.mode} • {new Date(s.created_at).toLocaleDateString()}</span>
                            </button>
                        ))}
                    </div>

                    <div className="pt-6 border-t border-black/5 dark:border-white/5">
                        <button onClick={handleSignOut} className="w-full p-4 rounded-2xl text-left text-red-500 hover:bg-red-50 dark:hover:bg-red-950/20 transition-all flex items-center gap-3">
            ...
            <div className="flex flex-col flex-1 min-w-0 h-full">
                <header className="flex-shrink-0 bg-transparent px-4 sm:px-8 py-4 flex items-center justify-between z-30 w-full">
                    <div className="flex items-center gap-4">
                        ...
                        >
                            <svg className="w-6 h-6 text-black dark:text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 6h16M4 12h16M4 18h16" /></svg>
                        </button>
                        <button
                            onClick={resetApp}
                        ...
                        >
                            ...
                            <div className="text-left truncate">
                                <h1 className="text-sm sm:text-lg font-bold text-black dark:text-white tracking-tight leading-none group-hover:text-gray-600 transition-colors truncate">Challenge Solver</h1>
                                <p className="hidden sm:block text-[10px] font-bold text-gray-400 tracking-[0.15em] uppercase mt-0.5">V3.5 Military</p>
                            </div>
                        </button>
                    </div>
